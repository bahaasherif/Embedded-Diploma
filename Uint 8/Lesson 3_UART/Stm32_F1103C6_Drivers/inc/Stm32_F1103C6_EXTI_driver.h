...
#ifndef INC_STM32_F1103C6_EXTI_DRIVER_H_
#define INC_STM32_F1103C6_EXTI_DRIVER_H_
...
typedef struct
{
	EXTI_GPIO_Mapping_t EXTI_PIN ; 				//must be set on @ref EXTI_define

	uint8_t 			EXTI_Trigger ;			//must be set on @ref EXTI_Trigger_define

	uint8_t				IRQ_EN ;				//must be set on @ref EXTRI_IRQ_define

	void (* P_IRQ_CallBack)(void) ;				//set the fucntion() which will be called when IRQ happen
...
#endif /* INC_STM32_F1103C6_EXTI_DRIVER_H_ */
